...
namespace NumberPlace.Algorithm
{
    public class MatFactory : IMatFactory
    {
        ...
        public Task<Mat> GenerateMatAsync(int initCount, int matIndex, CancellationToken token)
            => Task.Run(() => GenerateMat(initCount, matIndex), token);
        ...
        /// <param name="matIndex">数独编号</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Task<Mat> GenerateMatAsync(int initCount, int matIndex)
            => Task.Run(() => GenerateMat(initCount, matIndex));
        ...
        /// <param name="initCount"></param>
        /// <param name="matIndex">数独编号</param>
        public unsafe Mat GenerateMat(int initCount, int matIndex)
        {
            try
            {
            ...
            {
                OnMatGeneratedFailed?.Invoke(ex, matIndex);
                throw;
            }
        }
